		...
		for(size_t i = 0; i < interval_gcd; ++i){
			if(count[i] > 0){
				...
				for(char j = 0; j < 26; ++j){
					double sum = 0;
					for(char l = 'a'; l <= 'z'; ++l){
						sum += ref[l]/100 * freq[i][l+j] / count[i];
					}
					if(pow(sum - 0.0778, 2) < min_d){
						min_d = pow(sum - 0.0778, 2);
						min = j;
					}
				}

				std::cout << '#' << i << " substring index of coincidence: " << ciphertext_index << " probable key: " << (char)('a'+min) << std::endl;
